-- La puerta de escritura aprende con qué se generó el comentario (RF-1704).
--
-- Se sustituye la función en vez de añadir una segunda con más parámetros:
-- dos puertas para lo mismo es una que alguien olvidará mantener. Los dos
-- parámetros nuevos van al final y con valor por defecto, así que quien la
-- llamara con la forma anterior sigue funcionando.
--
-- `CREATE OR REPLACE` no vale aquí: cambiar la lista de parámetros crea una
-- **sobrecarga nueva** en vez de reemplazar, y quedarían las dos. Se borra la
-- anterior a mano, que además deja constancia de que era una y sigue siendo una.

DROP FUNCTION IF EXISTS agent_write_comment(uuid, uuid, uuid, text, uuid);--> statement-breakpoint
...
ALTER FUNCTION agent_write_comment(uuid, uuid, uuid, text, uuid, ai_provider, text)
  OWNER TO agent_writer;--> statement-breakpoint
REVOKE ALL ON FUNCTION agent_write_comment(uuid, uuid, uuid, text, uuid, ai_provider, text)
  FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION agent_write_comment(uuid, uuid, uuid, text, uuid, ai_provider, text)
  TO app_user;--> statement-breakpoint

COMMENT ON FUNCTION agent_write_comment(uuid, uuid, uuid, text, uuid, ai_provider, text) IS
